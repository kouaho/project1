...
<html lang="en">
    ...
    <head>
        <base href="../../../">
        <meta charset="utf-8" />
        <title>Bugtrucher | Liste des admin</title>
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <ul class="kt-nav">
                                                            <li class="kt-nav__section kt-nav__section--first">
                                                                <span class="kt-nav__section-text"> Choisir option</span>
                                                            </li>
                                    ...
                                    <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
                                        ...
                                        <tbody>
                                            @foreach ($users as $user)
                                                <tr>
                                                    <td>
                                                        <img src="{{URL::to('/')}}/img/user/{{$user->img}}" width="125px"></td>
                                                    <td>{{ $user->username }}</td>
                                                    <td>{{ $user->name }}</td>
                                                    <td>{{ $user->prenom }}</td>
                                                    <td>{{ $user->email }}</td>
                                                    ...
                                                    <td class="pull-center">
                                                        <form action="{{ route('users.destroy',$user->id) }}" method="POST">
                                                            <a class="btn btn-info" href="{{ route('users.show',$user->id) }}"><i class="flaticon-eye "></i>Voir</a>
                                                            <a class="btn btn-primary" href="{{ route('users.edit',$user->id) }}"><i class="flaticon-edit " ></i>Modifier</a>
                                                         @csrf
                                                         @method('DELETE')<button type="submit" class="btn btn-danger">Supprimer<i class="flaticon-delete"></i></button>
                                                      </form>
                                                    </td>
